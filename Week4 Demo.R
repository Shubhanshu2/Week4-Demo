...
# Assign newly structured date to the managers date field

managers$Date <- restructured_date
str(managers)

# Default date format in R is yyyy-mm-dd
# Define position of the date contents first in the imported data frame eg. using %d, %m etc
# R then puts the date field into yyyy-mm-dd format when using the date type
...
# Restructure the managers date field to the same structure as the include_list dataframe
...
new_managers_dataframe
str(new_managers_dataframe, 10)
